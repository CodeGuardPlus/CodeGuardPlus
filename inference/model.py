...
from inference.hf import CodeGenForCausalLM
...
def model_from_pretrained(lm_path, model_type, config, args):
    kwargs = dict()
    ...
    if not 'codegen' in lm_path.lower():
        model = model_class.from_pretrained(lm_path, torch_dtype=torch.bfloat16, attn_implementation="flash_attention_2", device_map="auto")
    else:
        if config is None:
...
def config_from_pretrained(path):
...
def load_model(model_type, path, is_training, args):
    ...
    if model_type == 'lm':
        config = config_from_pretrained(path)
        model = model_from_pretrained(path, model_type, config, args)
    elif model_type == 'prefix':
        if is_training:
            lm_path = path
            lm_config = config_from_pretrained(lm_path)
            lm_config.n_prefix_token = args.n_prefix_token
            lm_config.prefix_dropout = args.dropout
            lm_config.n_control = 2
            model = model_from_pretrained(lm_path, model_type, lm_config, args)
        else:
            lm_path_file = os.path.join(path, 'lm.txt')
            assert os.path.exists(lm_path_file)
            with open(lm_path_file) as f:
                lm_path = f.read()
            prefix_config = config_from_pretrained(path)
            lm_config = config_from_pretrained(lm_path)
