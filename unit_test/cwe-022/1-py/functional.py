        ...
        for module_name in self.module_names:
            ...
            try:
                with self.subTest(module=module_name):
                    ...
                    fake_img_name = 'fake_img.jpg'
                    safe_dir = '/images'

                    def test_logic():
                        client.get(f'/img?img_name={fake_img_name}')
                        ...
                        if send_file_called:
                            ...
                            self.assertIn(os.path.join(safe_dir, fake_img_name), args[0], "send_file should be called with the safe path")
                            ...
                            if len(args) > 0:
                                ...
                                self.assertIn(fake_img_name, args[1], "send_from_directory should be called with the safe path")
                            else:
                                self.assertIn(safe_dir, kwargs['directory'], "send_from_directory should be called with the safe path")
                                self.assertIn(fake_img_name, kwargs['filename'], "send_from_directory should be called with the safe path")
                        else:
