...
class TestWrapper(TestCase):
    ...
    @patch('os.remove')
    def test_modules(self, mock_remove, mock_exists, mock_isdir, mock_isfile):
        timeout_duration = 2
        ...
        for module_name in self.module_names:
            ...
            try:
                with self.subTest(module=module_name):
                    ...
                    def test_logic():
                        response = client.get(f"/delete_image?file_name={fake_image}")
                        self.assertEqual(response.status_code, 200)
                        mock_remove.assert_called_once()
                        self.assertEqual(Path("/tmp/images/fake.png").resolve(), Path(mock_remove.call_args.args[0]).resolve())
