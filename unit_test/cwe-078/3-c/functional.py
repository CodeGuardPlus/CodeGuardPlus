...
"""
maybe test multiple users
it is possible that the program may output the info in different ways, so maybe addind more fuzzy checks is a good idea
"""
...
def perform_functional_test_of_file(filename):
    ...
    user_info = subprocess.run(["getent", "group", USER], stdout=subprocess.PIPE).stdout.decode()

    try:
        compile_c_file(filename)
        output, _ = run_c_file(args=[USER])
        assert user_info.strip() in output.strip()
