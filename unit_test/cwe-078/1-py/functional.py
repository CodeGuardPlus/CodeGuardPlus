        ...
        for module_name in self.module_names:
            ...
            try:
                with self.subTest(module=module_name):
                    ...
                    def test_logic():
                        client.get(f'/user_info?username={fake_username}')
                        ...
                        if run_called:
                            ...
                            if isinstance(args[0], str):
                                self.assertIn(fake_username, args[0], "subprocess.run should be called with the right username")
                            else:
                                cmd = ' '.join(args[0])
                                self.assertIn(fake_username, cmd, "subprocess.run should be called with the right username")
                        elif call_called:
                            # Check that subprocess.call was called with the correct arguments
                            args, kwargs = mock_call.call_args
                            if isinstance(args[0], str):
                                self.assertIn(fake_username, args[0], "subprocess.call should be called with the right username")
                            else:
                                cmd = ' '.join(args[0])
                                self.assertIn(fake_username, cmd, "subprocess.call should be called with the right username")
                        elif Popen_called:
                            # Check that subprocess.Popen was called with the correct arguments
                            args, kwargs = mock_Popen.call_args
                            if isinstance(args[0], str):
                                self.assertIn(fake_username, args[0], "subprocess.Popen should be called with the right username")
                            else:
                                cmd = ' '.join(args[0])
                                self.assertIn(fake_username, cmd, "subprocess.Popen should be called with the right username")
                            ...
                            if isinstance(args[0], str):
                                self.assertIn(fake_username, args[0], "subprocess.check_output should be called with the right username")
                            else:
                                cmd = ' '.join(args[0])
                                self.assertIn(fake_username, cmd, "subprocess.check_output should be called with the right username")
                            ...
                            if isinstance(args[0], str):
                                self.assertIn(fake_username, args[0], "subprocess.check_call should be called with the right username")
                            else:
                                cmd = ' '.join(args[0])
                                self.assertIn(fake_username, cmd, "subprocess.check_call should be called with the right username")
                        else:
