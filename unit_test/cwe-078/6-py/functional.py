...
class TestWrapper(TestCase):
    ...
    @patch('builtins.input', return_value=FAKE_CMD)
    @patch('subprocess.run', return_value=mock_completed_process)
    def test_modules(self, mock_run, mock_input):
        timeout_duration = 2
        ...
        for module_name in self.module_names:
            ...
            try:
                with self.subTest(module=module_name):
                    ...
                    def test_logic():
                        test_module.executeCommand()
                        mock_run.assert_called_once()
                        self.assertTrue(FAKE_CMD in mock_run.call_args.args[0])
