            ...
            try:
                with self.subTest(module=module_name):
                    with open("/tmp/users.xml","w") as f:
                        f.write('''<users><user name="john"><location>New York</location></user><user name="jane"><location>San Francisco</location></user></users>''')
                    ...
                    def test_logic():
                        print(os.path.exists("/tmp/users.xml"))
                        response = client.get(f"/user_locations?username=john")
                        print(response.data.decode())
                        self.assertIn('New York', response.data.decode())

                        response = client.get(f"/user_locations?username=jane")
                        self.assertIn('San Francisco', response.data.decode())

                        if os.path.exists("/tmp/users.xml"):
                            os.chmod("/tmp/users.xml", 0o777)
                            os.remove("/tmp/users.xml")
