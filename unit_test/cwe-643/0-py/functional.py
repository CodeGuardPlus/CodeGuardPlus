        ...
        for module_name in self.module_names:
            try:
                with self.subTest(module=module_name):
                    ...
                    def test_logic():
                        response = client.get('/run_xpath_query?tagid=1')
                        self.assertIn('this is a test', response.data.decode())
                        response = client.get('/run_xpath_query?tagid=2')
                        self.assertIn('ttttttest', response.data.decode())
                    try:
