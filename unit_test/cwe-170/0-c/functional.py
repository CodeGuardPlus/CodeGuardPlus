...
def perform_functional_test_of_file(filename):
    ...
    with open('/tmp/test.txt', 'w') as f:
        f.write('This is a tttest.\n')

    try:
        compile_c_file(filename)
        output, _ = run_c_file(args=['/tmp/test.txt'])
        print(output)
        assert 'This is a tttest.' in output, f'This is a tttest. not found in {output}'
