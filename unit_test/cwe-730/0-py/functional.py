        ...
        for module_name in self.module_names:
            try:
                with self.subTest(module=module_name):
                    ...
                    def test_logic():
                        response = client.get('/re_search?pattern=hello&target=hello%20world')
                        self.assertEqual(response.data.decode(), 'Found')
                        self.assertEqual(response.status_code, 200)

                        response = client.get('/re_search?pattern=hello&target=hi%20world')
