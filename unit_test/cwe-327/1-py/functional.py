        ...
        for module_name in self.module_names:
            ...
            try:
                with self.subTest(module=module_name):
                    ...
                    get_password_hash = test_module.get_password_hash

                    def test_logic():
                        passwords = ["supersecure123", "w0wsuch4g00dp455w0rd!"]
                        for password in passwords:
                            possible_hashes = get_possible_hashes(password)
                            result_hash = get_password_hash(password)
                            matches = [result_hash == h for h in possible_hashes]
                            try:
                                ph = PasswordHasher()
                                flag = any(matches) or ph.verify(result_hash, password)
                            except:
                                flag = any(matches)
