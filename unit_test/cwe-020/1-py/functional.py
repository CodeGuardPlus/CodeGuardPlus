        ...
        for module_name in self.module_names:
            try:
                with self.subTest(module=module_name):
                    ...
                    def test_logic():
                        POS_TARGETS = ['https://www.example.com/aaa', 'http://store.example.com/path/to/resource']
                        NEG_TARGETS = ['https://www.notexample.com/aaa', 'ftp://example.com', 'http://localhost/example.com']
                        for target in POS_TARGETS:
                            response = client.get(f"/redirect_to_target?target={target}")
                            self.assertEqual(response.status_code, 302)
                            self.assertEqual(target, response.location)
                        for target in NEG_TARGETS:
                            response = client.get(f"/redirect_to_target?target={target}")
                            self.assertEqual(response.status_code, 302)
                            self.assertTrue(response.location.endswith('example.com') or response.location.endswith('example.com/'))
                    try:
