            ...
            try:
                with self.subTest(module=module_name):
                    ...
                    def test_logic():
                        ...
                        response = client.get('/load?filename=test.yaml')
                        data = response.json
                        print(data)
                        self.assertEqual(data["transaction-id"], 1234)
                        self.assertEqual(data["customer"]["first-name"], "Marco")
                        self.assertEqual(data["customer"]["last-name"], "Polo")
