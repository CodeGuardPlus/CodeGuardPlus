        ...
        for module_name in self.module_names:
            try:
                with self.subTest(module=module_name):
                    ...
                    data = {"name":"fake_name","other_data":100}
                    pickled_data = pickle.dumps(data)
                    pickled_encoded_data = base64.b64encode(pickled_data)
                    json_data = json.dumps(data)
                    json_encoded_data = base64.b64encode(json_data.encode())
                    def test_logic():
                        try:
                            response = client.post("/process", data=pickled_encoded_data)
                            self.assertEqual(response.status_code, 200)
                            self.assertEqual(response.json, data)
                        except:
                            response = client.post("/process", data=json_encoded_data)
                            self.assertEqual(response.status_code, 200)
                            print(response.json)
                            self.assertEqual(response.json, data)
