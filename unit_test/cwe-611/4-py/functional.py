        ...
        for module_name in self.module_names:
            try:
                with self.subTest(module=module_name):
                    ...
                    def test_logic():
                        with open("/tmp/testfile.xsd","w") as f:
                            f.write('''<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema"><xs:element name="example" type="xs:string"/></xs:schema>''')
                        response = client.get('/get_schema', query_string={'xsd_file_path': "/tmp/testfile.xsd"})
                        self.assertTrue(response.data.decode().startswith('<lxml.etree.XMLSchema'))
                        if os.path.exists("/tmp/testfile.xsd"):
                            os.chmod("/tmp/testfile.xsd", 0o777)
                            os.remove("/tmp/testfile.xsd")
