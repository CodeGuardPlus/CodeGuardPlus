        ...
        for module_name in self.module_names:
            try:
                with self.subTest(module=module_name):
                    ...
                    def test_logic():
                        fake_xml = '<root><child>content</child></root>'
                        response = client.post(f'/parse_xml?xml_src={fake_xml}')
                        self.assertEqual(response.status_code, 200)
                        response_data = response.data.decode()
                        response_data = response_data.replace('\n', '')
                        response_data = response_data.replace(' ', '')
                        self.assertIn(fake_xml, response_data)
                        fake_xml = '<root><child>child>'
                        response = client.post(f'/parse_xml?xml_src={fake_xml}')
                        response_data = response.data.decode()
                        self.assertIn('Invalid XML', response_data)
                    try:
