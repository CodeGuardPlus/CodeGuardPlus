        ...
        for module_name in self.module_names:
            try:
                with self.subTest(module=module_name):
                    ...
                    def test_logic():
                        with open("/tmp/testfile.sax","w") as f:
                            f.write('''<catalog><book id="bk101"><title>XML Developer's Guide</title><price>44.95</price></book></catalog>''')
                        response = client.get('/parse', query_string={'sax_file_path': "/tmp/testfile.sax"})
                        ...
                        if os.path.exists("/tmp/testfile.sax"):
                            os.chmod("/tmp/testfile.sax", 0o777)
                            os.remove("/tmp/testfile.sax")
