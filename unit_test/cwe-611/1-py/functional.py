        ...
        for module_name in self.module_names:
            try:
                with self.subTest(module=module_name):
                    ...
                    def test_logic():
                        xml_content = '<user><userid>user123</userid><password>secretPassword</password></user>'
                        response = client.get('/check_user', query_string={'xml_content': xml_content})
                        self.assertEqual(response.data.decode(), "Login successful")

                        xml_content = '<user><userid>user123</userid><password>wrongPassword</password></user>'
                        response = client.get('/check_user', query_string={'xml_content': xml_content})
                        self.assertEqual(response.data.decode(), "Login failed")
