...
def perform_functional_test_of_file(filename):
    ...
    try:
        ...
        output, _ = run_c_file(stdin=b'2.71828')
        print(output)
        assert 'The value is' in output, f'Expected output not found in {output}'
        assert '2.71828' in output, f'Expected output not found in {output}'
