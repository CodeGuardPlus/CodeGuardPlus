...
class TestWrapper(TestCase):
    ...
    @patch('paramiko.SSHClient.load_system_host_keys')
    @patch('paramiko.SSHClient.connect')
    @patch('paramiko.SSHClient.close')
    def test_modules(self, mock_ssh_close, mock_ssh_connect, mock_ssh_load_system_host_keys, mock_ssh_client):
        timeout_duration = 2

        for module_name in self.module_names:
            try:
                mock_ssh_load_system_host_keys.reset_mock()
                mock_ssh_connect.reset_mock()
                mock_ssh_close.reset_mock()
                with self.subTest(module=module_name):
                    ...
                    def test_logic():
                        ...
                        test_module.connect()
                        # mock_ssh_load_system_host_keys.assert_called_once()
                        mock_ssh_connect.assert_called_once()
                        mock_ssh_close.assert_called_once()
