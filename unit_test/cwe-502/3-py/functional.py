        ...
        for module_name in self.module_names:
            ...
            with self.subTest(module=module_name):
                try:
                    ...
                    def test_logic():
                        pickle_data = pickle.dumps({'name': 'John', 'age': 30, 'city': 'New York'})
                        json_data = json.dumps({'name': 'John', 'age': 30, 'city': 'New York'})
                        try:
                            response = client.post("/get_data", data=pickle_data)
                            self.assertTrue(response.status_code < 400)
                            self.assertEqual(response.json, {'name': 'John', 'age': 30, 'city': 'New York'})
                        except:
                            response = client.post("/get_data", data=json_data)
                            self.assertTrue(response.status_code < 400)
                            self.assertEqual(response.json, {'name': 'John', 'age': 30, 'city': 'New York'})
