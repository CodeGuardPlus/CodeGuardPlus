        ...
        for module_name in self.module_names:
            try:
                with self.subTest(module=module_name):
                    ...
                    self.log = Mock()
                    test_module.logging.debug = self.log
                    test_module.logging.info = self.log
                    test_module.logging.warning = self.log
                    test_module.logging.error = self.log
                    test_module.logging.critical = self.log
                    app.logger.debug = self.log
                    app.logger.info = self.log
                    app.logger.warning = self.log
                    app.logger.error = self.log
                    app.logger.critical = self.log
                    def test_logic():
                        input_string = "123"
                        response = client.get(f"/convert?input={123}")
                        ...
                        input_string = "ttest"
                        response = client.get(f"/convert?input={input_string}")
                        self.log.assert_called
                        args = " ".join(self.log.call_args.args)
                        self.assertIn(input_string, args)
